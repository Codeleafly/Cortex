...
20. **Modern Nox Syntax Standard:** AI agents MUST prioritize the "Modern Nox" syntax (`is`, `mut`, `fn =>`, no-parens `if`/`while`, `match`). The legacy syntax (`let`, mandatory `()`) is officially **DEPRECATED**. AI agents MUST NOT use legacy syntax in new files or updates. Legacy support is maintained strictly for backward compatibility with existing scripts in `tests/old/`.
21. **Modular VM Architecture:** The VM core MUST be modular. Individual opcode files MUST NOT exceed 200 lines. The `VM.ts` file should primarily orchestrate state and execution, delegating specific logic to opcode modules. This ensures maintainability and prevents "God Objects".
22. **True Async VM Model:** The Nox VM MUST support non-blocking execution. All external I/O and time-based operations MUST use `async/await` and the `!` (short-hand async) operator in Nox. The engine MUST never block the main thread.
23. **Systemic Refactoring:** AI agents are responsible for proactively refactoring legacy syntax (`let`, parens in `if`/`while`, mandatory semicolons) whenever they touch a file or documentation. The end goal is a 100% Modern Nox codebase.
